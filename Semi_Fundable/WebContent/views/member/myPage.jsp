...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="com.fun.product.model.vo.Image ,com.fun.member.model.vo.Pay, com.fun.member.model.vo.Member , java.util.ArrayList, java.text.DecimalFormat,java.text.NumberFormat,java.util.Locale"%>
    <%
	...
	nav{
      ...
      body{
       ...
       #funding-history-header5{
		...
		#funding-history a:hover{
			text-decoration:underline !important;
			color:blue;
		}
		#mypage-content table tbody tr:hover{
...
<div align="right" id="push-detail">
  <form action="<%= contextPath %>/notification.me" method="post">
  <button type="button" class="btn btn-outline-secondary " onclick="" style="visibility:hidden;">
...
<div id="mypage-content">
  ...
  <div id="fun-wrap">
    ...
    <div align="right" id="date-search" >
      ...
      <input type="date" name="startDate" id=""> ㅡ
    ...
    <table >
        ...
        <tbody>
                  ...
                  <tr  <%if(pTextList.get(i).getrStatus().equals("N") || pTextList.get(i).getrStatus().equals("C")){ %> style="background-color:rgb(235, 235, 235); color:gray;" <%} %>>
                    ...
                    <td id="mypage-fun-state"><!--펀딩진행상황--><% if(pTextList.get(i).getfStatus().equals("Y") && pTextList.get(i).getrStatus().equals("Y") ){  %>
                    	진행중
                    <%} else if(pTextList.get(i).getrStatus().equals("N")){%>
                    	펀딩취소
                    <%} else{%>
                    	펀딩종료
                    <%} %>
                      ...
                      <td >
                        ...
                        <div id="mypage-product-textwrap">
                          <p id="mypage-product-brand" ><!--브랜드명--> <a <% if(pTextList.get(i).getrStatus().equals("Y")){ %> href=""  <%} %> id="pb" <%if(pTextList.get(i).getrStatus().equals("N") || pTextList.get(i).getrStatus().equals("C")){ %> style="color:gray; text-decoration:none;" <%} %>><%= pTextList.get(i).getSellerNickName() %></a></p>
                        <span id="mypage-product-title"><!--상품명--> <a <% if(pTextList.get(i).getrStatus().equals("Y")){ %> href="<%=contextPath %>/fsi.bo?productNo=<%= pTextList.get(i).getProductNo()%>"  <%} %> <%if(pTextList.get(i).getrStatus().equals("N") || pTextList.get(i).getrStatus().equals("C")){ %> style="color:gray; text-decoration:none;" <%} %>><%= pTextList.get(i).getProductTitle()%></a></span>
                        <p id="mypage-product-option"><!--옵션-->옵션 : <%= pTextList.get(i).getOptionName() %>&nbsp; <br> 결제금액 : <%=df.format(pTextList.get(i).getTotalPrice())%>원 </p>
                        </div>
                      </div>
                      </td>
                      <td><a id="mypage-product-orderno" <% if(pTextList.get(i).getrStatus().equals("Y")){ %> data-bs-toggle="modal" data-bs-target="#exampleModa<%= i %>" <%} %>style="font-size: 0.8em; "><!-- 주문번호-->20239999<%= pTextList.get(i).getOrederNo() %></a></td>
                      <td style="font-size: 0.8em; font-weight: bold;"><!--참여일--><%= pTextList.get(i).getFundingDate() %></td>
                      <td style="font-size: 0.8em; font-weight: bold;"><!--종료일--><%= pTextList.get(i).getEndDate() %></td>
                      
                    </tr>
                    <%} %>
                   
                    <%}else{ %>
						<td colspan="5">펀딩 참여 내역이 없습니다</td>
					<%} %>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <img src="<%= contextPath %>/<%= pList.get(i).getFilePath() + pList.get(i).getChangeName() %>" id="modal-img"></a>
          <div style="margin-top: 2vh; padding-left: 1vw; width:100%" >
            <p id="mypage-product-brand" style="margin-bottom: 0.6vh;"><!--브랜드명-->
            <a href="" style="font-size: 15px;" id="modal-brand"><%= pTextList.get(i).getSellerNickName() %></a></p>
          <span id="mypage-product-title"><!--상품명-->
          <a href="<%=contextPath %>/fsi.bo?productNo=<%= pTextList.get(i).getProductNo()%>" style="font-size: 16px;" id="modal-title"><%= pTextList.get(i).getProductTitle() %></a></span>
          <p id="mypage-product-option" style="font-size: 11px; margin-top: 1vh; margin-bottom: 1.5vh;">
          <!--옵션-->옵션 : <%= pTextList.get(i).getOptionName() %> &nbsp; <br>
          결제금액 : <%= df.format(pTextList.get(i).getTotalPrice()) %>원</p>
          <form align="right" action="test.do" method="post">
          <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#exampleModalToggleLabe<%= i%>">예약 취소</button>
          </form>
          </div>
        </div>
        ...
        <div class="modal-body" style="border-top: 1px solid rgb(216, 215, 215);">
          <table align="right">
            ...
            <tr>
              ...
              <div  style="display: flex; justify-content:flex-end;  height:15vh; align-items:end"><button type="button" class="btn btn-secondary" onclick="location.href='<%= contextPath %>/address.me'">배송지 변경</button></div>
            </td>
            </tr>
          </table>
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
       ...
       <!--   <input type="hidden" name="hiddenRequest" value="refund"> -->
         ...
         <input type="hidden" name="hiddenRequest" value="refundProduct">
